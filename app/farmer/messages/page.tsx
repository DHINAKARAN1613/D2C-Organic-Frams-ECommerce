...
import { Send, Loader2, MessageSquare, User as UserIcon, Search, Users } from 'lucide-react';
...
export default function FarmerMessagesPage() {
    ...
    useEffect(() => {
        if (selectedSessionId) {
            ...
            const interval = setInterval(fetchSelectedSession, 8000);
            return () => clearInterval(interval);
        }
    }, [selectedSessionId]);
        ...
        <div className="h-[calc(100vh-10rem)] bg-[#1c2e24] border border-[#2d4035] rounded-2xl overflow-hidden flex shadow-xl">
            <div className="w-1/3 md:w-80 border-r border-[#2d4035] flex flex-col bg-[#112117]/50">
                <div className="p-4 border-b border-[#2d4035]">
                    <h2 className="text-lg font-bold text-white mb-4">{t.customerMessages}</h2>
                </div>
                                ...
                                <button key={chat.id} onClick={() => setSelectedSessionId(chat.id)} className={`w-full text-left p-4 hover:bg-[#1c2e24] transition-colors ${selectedSessionId === chat.id ? 'bg-[#1c2e24] border-l-4 border-[#30e87a]' : 'border-l-4 border-transparent'}`}>
                                    <div className="flex items-center gap-3">
                                        <div className="size-10 rounded-full bg-[#112117] border border-[#2d4035] flex items-center justify-center overflow-hidden shrink-0">
                                            {getImageUrl(chat.user?.image) ? <img src={getImageUrl(chat.user.image)!} alt="Customer" className="w-full h-full object-cover" /> : <UserIcon className="w-5 h-5 text-[#9db8a8]" />}
                                        </div>
                                        <div className="flex-1 min-w-0 hidden md:block">
                                            <div className="flex justify-between items-baseline mb-1">
                                                <p className="text-sm font-bold text-white truncate">{chat.user?.name || t.customer}</p>
                                            </div>
                ...
                {selectedSessionId && selectedSession ? (
                    <>
                        <div className="p-4 bg-[#1c2e24] border-b border-[#2d4035] flex items-center gap-3">
                            <div className="size-10 rounded-full bg-[#112117] border border-[#2d4035] flex items-center justify-center overflow-hidden shrink-0">
                                {getImageUrl(selectedSession.user?.image) ? <img src={getImageUrl(selectedSession.user.image)!} alt="Customer" className="w-full h-full object-cover" /> : <UserIcon className="w-5 h-5 text-[#9db8a8]" />}
                            </div>
                            <div><h3 className="font-bold text-white">{selectedSession.user?.name || t.customer}</h3></div>
                        </div>
                        ...
                        <div className="p-3 md:p-4 bg-[#1c2e24] border-t border-[#2d4035]">
                            <form onSubmit={handleSend} className="relative flex items-center max-w-4xl mx-auto">
                                <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder={t.replyToCustomer} className="w-full bg-[#112117] border border-[#2d4035] rounded-full pl-5 pr-14 py-3 md:py-4 text-sm text-white placeholder-[#9db8a8] focus:outline-none focus:border-[#30e87a] transition-colors shadow-inner" />
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-[#9db8a8]">
                        ...
                        <h3 className="text-xl font-bold text-white mb-2">{t.customerInbox}</h3>
                        <p className="max-w-xs text-center text-sm">{t.selectCustomerChat}</p>
                    </div>
                )}
            </div>
        </div>
