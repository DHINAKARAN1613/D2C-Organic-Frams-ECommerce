                ...
                <div className="flex-1 overflow-y-auto">
                    {sessions.length === 0 ? (
                        <div className="p-8 text-center text-[#9db8a8]">
                            ...
                            <p className="text-sm">No messages from customers yet.</p>
                        </div>
                    ) : (
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex-1 min-w-0 hidden md:block">
                                            <div className="flex justify-between items-baseline mb-1">
                                                <p className="text-sm font-bold text-white truncate">{chat.user?.name || 'Customer'}</p>
                                            </div>
                ...
                {selectedSessionId && selectedSession ? (
                    <>
                        <div className="p-4 bg-[#1c2e24] border-b border-[#2d4035] flex items-center gap-3">
                            ...
                            <div><h3 className="font-bold text-white">{selectedSession.user?.name || 'Customer'}</h3></div>
                        </div>
                        ...
                        <div className="p-3 md:p-4 bg-[#1c2e24] border-t border-[#2d4035]">
                            <form onSubmit={handleSend} className="relative flex items-center max-w-4xl mx-auto">
                                <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Reply to customer..." className="w-full bg-[#112117] border border-[#2d4035] rounded-full pl-5 pr-14 py-3 md:py-4 text-sm text-white placeholder-[#9db8a8] focus:outline-none focus:border-[#30e87a] transition-colors shadow-inner" />
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-[#9db8a8]">
                        ...
                        <h3 className="text-xl font-bold text-white mb-2">Customer Inbox</h3>
                        <p className="max-w-xs text-center text-sm">Select a customer chat from the sidebar to reply.</p>
                    </div>
                )}
            </div>
        </div>
