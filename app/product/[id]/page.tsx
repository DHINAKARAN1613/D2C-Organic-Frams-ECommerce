...
// Helper to map icon name to component (since lucide is used)
const IconMap = {
    public: Globe,
    eco: Leaf,
...
export default function ProductPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        const fetchRecommendations = async () => {
            if (!id) return;
            try {
                const res = await fetch(`/api/recommendations?productId=${id}`);
    ...
    useEffect(() => {
        // Only run randomization on the client to avoid hydration mismatch
        if (id) {
            setRelatedProducts(
    ...
    return (
        <div className="flex-grow w-full font-display bg-background min-h-screen text-foreground pt-24 pb-12 overflow-x-hidden">
            <main className="container mx-auto flex grow flex-col px-4 md:px-6 lg:px-8 py-8">
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-2">
                                    <div className="flex text-primary">
                                        {[...Array(5)].map((_, i) => (
                                            <Star key={i} className={`w-[18px] h-[18px] ${i < Math.floor(product.rating) ? 'fill-current' : 'text-muted-foreground'}`} />
                                        ))}
                                    </div>
                                    <span className="text-sm font-medium text-muted-foreground">({product.rating}/5.0 from {product.reviews} reviews)</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-y-4 gap-x-8 text-sm">
                            {product.attributes?.map((attr, idx) => (
                                <div key={idx} className="flex items-start gap-3">
                                    ...
                                    // Fallback attributes if none defined
                                    <>
                ...
                {/* AI Recommendations: Frequently Bought Together (Apriori Algorithm) */}
